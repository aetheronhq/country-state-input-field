...
def should_ignore(file_path, ignore_list):
    for pattern in ignore_list:
        if fnmatch.fnmatch(file_path, pattern):
            print(f"Ignoring {file_path} (matched pattern: {pattern})")
            return True
    return False
...
if __name__ == "__main__":
    ...
    ignore_file_path = os.path.join(repo_path, "../scripts/.gptignore")
    if sys.platform == "win32":
    ...
    if not os.path.exists(ignore_file_path):
        ...
        print(f"Warning: .gptignore file not found. Using {ignore_file_path} instead.")
